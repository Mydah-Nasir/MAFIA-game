...
def ended():
    ...
    global goto
    goto=Button(window,text='Exit Game',font=('Freestyle Script',30,'underline'),width=500,fg='#242424',bg='#9c9c9c',borderwidth=0,command=exit1)
...
def goback():
    ...
    number.place_forget()
    back_button.pack_forget()
    rules.pack_forget()      #removing all widgets when the go back button is clicked
    button_selection.place_forget()
    rules_button.place_forget()
    global label_story
...
mafia_menu=Menu(window)
window.config(menu=mafia_menu)
mafia_file=Menu(mafia_menu)
mafia_menu.add_cascade(label='File',menu=mafia_file)
mafia_file.add_command(label='Exit',command=exit1)
